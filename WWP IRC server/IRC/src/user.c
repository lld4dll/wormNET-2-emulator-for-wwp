...
register_local_user(struct Client *source_p)
{
  ...
  {
    char url[5][100] = {"wormnet.team17.com$1", "wormnet.team17.com$2", "wormnet.team17.com$3", "wormnet.team17.com$4", "wormnet.team17.com$5"};
  uint8_t key[] = "\xD5\x2D\x31\x08\xFB\x0F\x54\x9E\x6D\x7A\x0F\xFD\xEE\xDC\x21\x9A\xD4\xA6\x84\x24\x6D\x61\xFA\x8A\xAE\x98\x96\xA1\xF1\x63\x1A\x8D";
  uint8_t text[100];
  uint8_t iv[16];
  uint8_t hash[20];
  memset(iv, '\0', 16);
  ...
  memcpy(source_p->authping, text, strlen(text));
    sendto_one(source_p, "AUTHPING %s %s ", source_p->url, source_p->authping); 
  return; 
  }
